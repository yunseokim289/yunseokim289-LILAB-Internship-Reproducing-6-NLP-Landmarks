...
def writer_agent(state: AgentState):
    ...
    if iteration == 0:
        print("Writer 에이전트가 첫번째 초안을 작성하고 있습니다.")
        # 주어진 주제에 대해 2문장짜리 짧은 소개글을 쓰라고 지시
        prompt = f"[INST] Write a short, 2-sentence introduction about {state['topic']}. [/INST]"
    else:
...
workflow.add_edge(START, "Writer") # 시스템이 시작(START)되면 무조건 "Writer" 노드부터 실행되도록 선을 그음
workflow.add_edge("Writer", "Reviewer") # Writer가 자기의 역할을 다 하면 Reviewer에게 리턴값을 전달 (★멀티 에이전트의 핵심)
...
if __name__ == "__main__":
    ...
    # 이 상태가 START->"Writer"->"Reviewer"->conditional_router 거쳐서 빠져나옴
    result = app.invoke({"topic": test_topic, "iteration" : 0})

    # 그래프를 싹 다 돌고 최종적으로 튀어나온 결과물의 'topic', 'draft', 'review' 칸을 확인해서 출력합니다
