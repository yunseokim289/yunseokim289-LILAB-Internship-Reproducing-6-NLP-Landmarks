...
## 4. 실험 결과 (Ablation Study: Prompting)
까다로운 영화 리뷰 문장("The acting was okay, but the plot made absolutely no sense and I fell asleep.")을 타겟으로 긍정/부정 분류 실험 진행.

* [Test 1] Zero-shot Prompting
  * 결과: `Negative` (정답)
  * 분석: 예시 없이 지시사항만으로도 문맥의 뉘앙스를 정확히 파악함.
* [Test 2] One-shot Prompting
  * 결과: `Negative` (정답)
  * 분석: 1개의 긍정 리뷰 예시를 완벽하게 학습하여 출력함.
* [Test 3] Few-shot (5-shot) Prompting
  * 결과: `Negative (for the last review)` (정답)
  * 분석: 정답은 정확히 맞췄으나, 예시가 길어지자 모델이 단답형 지시("Only answer with one word")를 일부 무시하고 추가적인 설명을 덧붙이는 흥미로운 LLM의 특성이 관찰됨. 
