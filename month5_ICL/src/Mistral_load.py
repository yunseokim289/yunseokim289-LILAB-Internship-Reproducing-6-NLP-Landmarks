...
# SST-2 감성분류 벤치마트 데이터셋의 일부(100개)만 문맥에 넣어서 prompting하여 
# correct_count를 계산하는(=채점하는) 자동화 채점 루프를 돌려서 accuracy란 수치(최종 점수)를 뽑아냄

# 검증할 벤치마크 데이터셋 100개만 가져오기
dataset = load_dataset("glue","sst2", split = "validation").select(range(100))

print("100개 데이터셋 Zero-shot 자동 채점 시작")
correct_count = 0

# tqdm은 반복문이 한 바퀴 돌 때마다 걸리는 시간을 뒤에서 몰래 계산해서 자동으로 터미널에 찍어줌
# 이걸로 처리 속도 파악 가능
for item in tqdm(dataset):
    review = item['sentence']
    # 정답: 1이면 Positive, 0이면 Negative
    actual_label = "Positive" if item['label'] == 1 else "Negative"
    ...
    # 텍스트 프롬프트를 숫자로 쪼개서 (Tokenize) GPU로 보냄
    ...
    # 모델이 뱉어낸 숫자들을 다시 사람이 읽을 수 있는 영어로 번역 (Decode)
    # skip_special_tokens = True : 쓸데없는 기호는 빼고 주라고 지시
    response = tokenizer.decode(outputs[0], skip_special_tokens = True)

    # 프롬프트 부분을 잘라내고 모델이 순수하게 대답한 부분만 추출
    ...
    # 정답 체크 
    if actual_label.lower() in answer.lower(): # 모델 대답에 정답이 포함되어 있으면
        correct_count += 1

# 최종 점수 출력
accuracy = (correct_count / 100) * 100
print(f"최종 Zero-shot Accuracy: {accuracy}% ({correct_count} / 100)")

# 7. 5-shot (few-shot) prompting

print("100개 데이터셋 few-shot 자동 채점 시작")
correct_count = 0
...
# 최종 점수 출력
accuracy = (correct_count / 100) * 100
print(f"최종 few-shot Accuracy: {accuracy}% ({correct_count} / 100)")
